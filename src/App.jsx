...
export default function App() {
  ...
  return (
    <div className="bg-black text-white min-h-screen font-sans antialiased selection:bg-green-500 selection:text-black">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-16 lg:py-24">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {features.map((feature, index) => (
            <div key={index} className="bg-zinc-900/40 border border-zinc-800 rounded-2xl sm:rounded-3xl p-6 sm:p-8 hover:border-green-500 transition-colors">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-16 lg:py-24">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            {products.map((product, index) => (
              <div key={index} className="bg-black border border-zinc-800 rounded-2xl sm:rounded-3xl overflow-hidden hover:border-green-500 transition-colors flex flex-col justify-between">
                <div className="h-44 sm:h-56 bg-zinc-900/30 flex items-center justify-center text-5xl sm:text-6xl border-b border-zinc-800">
                  🏗️
                </div>
                <div className="p-6 sm:p-8 flex-grow flex flex-col justify-between">
                  <div>
                    <h3 className="text-xl sm:text-2xl font-bold mb-3">{product.name}</h3>
                    <p className="text-zinc-400 text-sm sm:text-base leading-relaxed mb-6">{product.desc}</p>
                  </div>
                  <a href="#configurator" className="block w-full text-center bg-white text-black py-3 sm:py-4 rounded-xl sm:rounded-2xl font-semibold hover:bg-zinc-200 transition-colors text-sm sm:text-base active:scale-98">
                    查看配置
                  </a>
                </div>
              </div>
      ...
      <section id="configurator" className="max-w-7xl mx-auto px-4 sm:px-6 py-16 lg:py-24 scroll-mt-6">
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-10">
          <div className="bg-zinc-900/30 border border-zinc-800 rounded-2xl sm:rounded-3xl p-6 sm:p-8">
            <h3 className="text-xl sm:text-2xl font-bold mb-6">選配項目</h3>
            <div className="space-y-3 sm:space-y-4">
              {optionList.map((option) => (
                <label key={option.id} className={`flex items-center justify-between border rounded-xl sm:rounded-2xl p-4 transition-all cursor-pointer select-none ${selectedOptions[option.id] ? 'border-green-500 bg-green-950/10' : 'border-zinc-800 hover:border-zinc-700'}`}>
          ...
          <div className="bg-zinc-900/30 border border-zinc-800 rounded-2xl sm:rounded-3xl p-6 sm:p-8 flex flex-col justify-between h-full">
            <div>
              <h3 className="text-xl sm:text-2xl font-bold mb-6">即時報價總覽</h3>
              <div className="space-y-4 sm:space-y-6">
                ...
                {optionList.map(opt => selectedOptions[opt.id] && (
                  <div key={opt.id} className="flex justify-between text-green-400 text-xs sm:text-sm">
                    <span>➕ {opt.name}</span>
                    <span className="font-mono">+NT$ {opt.price.toLocaleString()}</span>
                  </div>
                ))}
                <div className="border-t border-zinc-800 pt-4 sm:pt-6 flex justify-between text-2xl sm:text-3xl font-bold">
                  <span>總價</span>
      ...
      {/* PATENT */}
      <section className="bg-zinc-950 border-y border-zinc-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-16 lg:py-24 grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          <div className="text-center lg:text-left">
            <h2 className="text-3xl sm:text-4xl font-bold mb-6">專利技術保障</h2>
            <div className="space-y-4 sm:space-y-6 text-zinc-400 leading-relaxed text-base sm:text-lg max-w-xl mx-auto lg:mx-0">
          ...
          <div className="relative aspect-video rounded-2xl sm:rounded-3xl overflow-hidden border border-zinc-800 bg-gradient-to-bl from-zinc-900 via-black to-zinc-900 flex flex-col justify-center items-center p-6 text-center">
            ...
            <p className="text-zinc-500 text-xs sm:text-sm mt-2">高強度碳鋼方柱骨架與翼展連桿結構設計</p>
          </div>
        </div>
      </section>
      ...
      <section id="investment" className="max-w-7xl mx-auto px-4 sm:px-6 py-16 lg:py-24 text-center">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid sm:grid-cols-3 gap-4 sm:gap-8 mt-12 sm:mt-16">
            <div className="bg-zinc-900/30 border border-zinc-800 rounded-2xl sm:rounded-3xl p-6 sm:p-8">
              <div className="text-3xl sm:text-4xl font-bold mb-2 text-green-400">2年</div>
              <p className="text-zinc-500 text-xs sm:text-sm">預估投資回報期</p>
            </div>
            <div className="bg-zinc-900/30 border border-zinc-800 rounded-2xl sm:rounded-3xl p-6 sm:p-8">
              <div className="text-3xl sm:text-4xl font-bold mb-2 text-green-400">25%</div>
              <p className="text-zinc-500 text-xs sm:text-sm">年化節能與綜合收益率</p>
            </div>
            <div className="bg-zinc-900/30 border border-zinc-800 rounded-2xl sm:rounded-3xl p-6 sm:p-8">
              <div className="text-3xl sm:text-4xl font-bold mb-2 text-green-400">100%</div>
              <p className="text-zinc-500 text-xs sm:text-sm">全天候綠電自給自足率</p>
            </div>
          </div>
        </div>
      </section>
