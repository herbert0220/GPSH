...
export default function App() {
  ...
  const optionList = [
    { id: 'glass', name: '落地玻璃門與隔音窗', price: 45000 },
    ...
    <div className="bg-black text-white min-h-screen font-sans antialiased">
          ...
          <div className="relative aspect-video rounded-2xl sm:rounded-3xl overflow-hidden border border-zinc-800 bg-zinc-900 flex flex-col justify-center items-center shadow-2xl">
            <img 
              ...
              onError={(e) => {
                ...
                // 如果抓不到上傳的圖片，自動啟用高端幾何科技占位圖，絕不顯示醜陋的破圖圖標
                e.target.parentNode.innerHTML = `
                  <div className="absolute inset-0 bg-gradient-to-br from-zinc-900 via-black to-green-950/30 flex flex-col justify-center items-center p-6 text-center w-full h-full">
                    <span className="text-4xl sm:text-5xl mb-3">🏠</span>
                    <h4 className="text-base sm:text-xl font-bold text-green-400 font-mono">GPSH REAL MODEL</h4>
                    <p className="text-zinc-500 text-xs sm:text-sm mt-2">請於 GitHub 根目錄上傳名為 house-main.png 的實體照片</p>
                  </div>
                `;
              }}
            />
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-16 lg:py-24">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {features.map((feature, index) => (
            <div key={index} className="bg-zinc-900/60 border border-zinc-800 rounded-2xl p-6 hover:border-green-500 transition-colors">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {products.map((product, index) => (
              <div key={index} className="bg-black border border-zinc-800 rounded-2xl overflow-hidden flex flex-col justify-between">
                <div className="h-44 bg-zinc-900/50 flex items-center justify-center text-5xl border-b border-zinc-800">🏗️</div>
      ...
      <section id="configurator" className="max-w-7xl mx-auto px-4 sm:px-6 py-16 lg:py-24 scroll-mt-6">
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          <div className="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-6">
            ...
            <div className="space-y-3">
              {optionList.map((option) => (
                <label key={option.id} className={`flex items-center justify-between border rounded-xl p-4 transition-all cursor-pointer select-none ${selectedOptions[option.id] ? 'border-green-500 bg-green-950/20' : 'border-zinc-800 hover:border-zinc-700'}`}>
          ...
          <div className="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-6 flex flex-col justify-between h-full">
            <div>
      ...
      <section className="bg-zinc-950 border-y border-zinc-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-16 lg:py-24 grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="relative aspect-video rounded-2xl overflow-hidden border border-zinc-800 bg-gradient-to-bl from-zinc-900 via-black to-zinc-900 flex flex-col justify-center items-center p-6 text-center">
            ...
            <h4 className="text-base font-bold text-zinc-400 font-mono">STRUCTURE SCHEMA</h4>
